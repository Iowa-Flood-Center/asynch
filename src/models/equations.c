...
// combines model 405 and 406 . has 3 modes. passive , active calculating opening by rules, and active using opening as input
void model408(double t,
              const double *const y_i,
              unsigned int dim,
              const double *const y_p,
              ...
              double *ans)
{
    ...
    double frozen_ground = forcing_values[3];                                 // 1 if ground is frozen, 0 if not frozen
    double x1 = 0;
    double output_mode = global_params[11];
    ...
    unsigned int STATE_SUBSURF = 3;
    unsigned int STATE_GW = 4;
    unsigned int STATE_DAM_STORAGE = 5;
    unsigned int STATE_PIPE_FLOW = 6;
    unsigned int STATE_WEIR_FLOW = 7;
    unsigned int STATE_SNOW = 8;
    unsigned int STATE_OPENING_PERCENTAGE = 9;
    ...
    double q = y_i[STATE_DISCHARGE];                                   //[m^3/s]
    double dam_storage = y_i[STATE_DAM_STORAGE];                       // m^3
    double h6 = y_i[STATE_PIPE_FLOW];                                  // m3s-1?
    double h7 = y_i[STATE_WEIR_FLOW];                                  // m3s-1?
    ...
    { // dams
        ...
        double rating_flow = 0; // m3s-1
        // double output_mode=2; //defines reservoir operation scheme. 0 = passive. 1 = conservation pools. 2=incremental outflow increases with storage
        double pipe_flow = 0; // m3/s
        double weir_flow = 0; // m3/s
        ...
        // if(dam_storage + dam_input >0) debug=1; //only print results if there is water in the pond
        if (debug)
            printf("max.sto.pond %f\n", max_storage_pond);
        if (debug)
            printf("low_storage_factor and high_storage_factor: %f and %f\n", factor_low_threshold, factor_high_threshold);

        if (debug)
            printf("low_storage_threshold and high_storage_threshold: %f and %f\n", low_storage_threshold, high_storage_threshold);
        if (dam_storage + dam_input >= max_storage_pond)
        {
            if (debug)
                    printf("storage >= max.sto.pond\n");
            // S_max = qvs->points[qvs->n_values - 1][0];
            if (debug)
                    printf("qmax : %f\n", q_max);
            dam_outflow = q_max; // m3s-1
            rating_flow = dam_outflow;
            // if(debug1) printf("rating %f\n", rating_flow);
        }
        else if (dam_storage + dam_input < max_storage_pond)
        {
            if (debug)
                    printf("storage within rating range\n");
            double S = (dam_storage + dam_input < 0.0) ? 0.0 : dam_storage + dam_input;
            if (debug)
                    printf("storage  : %f\n", S);
            // find discharge value in rating
            ...
            {
                    // if(debug)printf("model403 storage qvs points: %f\n", qvs->points[ii][0]);
                    if (qvs->points[ii][0] <= S && S < qvs->points[ii + 1][0])
                        break;
            }
            double q2 = qvs->points[ii + 1][1];
            double q1 = qvs->points[ii][1];
            ...
            // Caculating the estimate of the pond storage below the weir
            for (ii = 0; ii < qvs->n_values - 1; ii++)
            {
                    // if(debug)printf("model403 storage qvs points: %f\n", qvs->points[ii][0]);
                    if (qvs->points[ii][1] >= 1)
                        break;
            }
            double q4 = qvs->points[ii][1];
            double q3 = qvs->points[ii - 1][1];
            double S4 = qvs->points[ii][0];
            double S3 = qvs->points[ii - 1][0];
            // dam_outflow = (q2 - q1) / (S2 - S1) * (S - S1) + q1;//m3s-1
            // rating_flow is Qrating in the flowchart
            double weir_stor = ((S3 * (q4 - 1)) + (S4 * (1 - q3))) / (q4 - q3); // m3
            ...
            if (output_mode == 1) 
            {
                    if (rating_flow <= 1)
                    {
                        dam_outflow = rating_flow * opening_percentage_initial;
                        ans[STATE_OPENING_PERCENTAGE] = 0;

                        ans[STATE_PIPE_FLOW] = dam_outflow - h6; // m3/s

                        if (h7 <= 0)
                        {
                            ans[STATE_WEIR_FLOW] = 0;
                        }
                        if (h7 > 0)
                        {
                            ans[STATE_WEIR_FLOW] = h7 - h7;
                        }
                    } // ratinflow<1
                    ...
                    if (rating_flow > 1)
                    {
                        ...
                        {
                            dam_outflow = rating_flow;
                            weir_flow = dam_outflow - 1;
                            ans[STATE_PIPE_FLOW] = 1 - h6; // m3/s (the maximum flow from the pipe is 1 m3/s)
                            ans[STATE_WEIR_FLOW] = weir_flow - h7;
                            // if(debug1) printf("on the rating");
                        }
                        ...
                        if (dam_storage + dam_input >= max_storage_pond)
                        {
                            ...
                            if (debug1)
                                printf("off rating");
                            // if(debug1) printf("dam out : %f\n", dam_outflow);
                            // if(debug1) printf("weir flow : %f\n", weir_flow);
                            // if(debug1) printf("weir flow : %f\n", weir_flow);
                        }
                    } // ratingflow>1
            }         // outputmode1

            // ACTIVE ,CALCULATING OPENING
            if (output_mode == 2)
            {

                    // For flows that are below the weir (100% through the pipe)
                    if (rating_flow <= 1)
                    {
                        double opening_percentage = 0;
                        if (dam_storage + dam_input <= (weir_stor * 0.05))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.05) && (dam_storage + dam_input <= (weir_stor * 0.1)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.1) && (dam_storage + dam_input <= (weir_stor * 0.15)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.15) && (dam_storage + dam_input <= (weir_stor * 0.2)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.2) && (dam_storage + dam_input <= (weir_stor * 0.25)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.25) && (dam_storage + dam_input <= (weir_stor * 0.3)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.3) && (dam_storage + dam_input <= (weir_stor * 0.35)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.35) && (dam_storage + dam_input <= (weir_stor * 0.4)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.4) && (dam_storage + dam_input <= (weir_stor * 0.45)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.45) && (dam_storage + dam_input <= (weir_stor * 0.5)))
                        {
                            opening_percentage = 0.05;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.5) && (dam_storage + dam_input <= (weir_stor * 0.55)))
                        {
                            opening_percentage = 0.10;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.55) && (dam_storage + dam_input <= (weir_stor * 0.6)))
                        {
                            opening_percentage = 0.30;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.6) && (dam_storage + dam_input <= (weir_stor * 0.65)))
                        {
                            opening_percentage = 0.30;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.65) && (dam_storage + dam_input <= (weir_stor * 0.7)))
                        {
                            opening_percentage = 0.50;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.7) && (dam_storage + dam_input <= (weir_stor * 0.75)))
                        {
                            double opening_percentage = 0.50;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.75) && (dam_storage + dam_input <= (weir_stor * 0.8)))
                        {
                            opening_percentage = 0.70;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.8) && (dam_storage + dam_input <= (weir_stor * 0.85)))
                        {
                            opening_percentage = 0.70;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.85) && (dam_storage + dam_input <= (weir_stor * 0.9)))
                        {
                            opening_percentage = 0.90;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.9) && (dam_storage + dam_input <= (weir_stor * 0.95)))
                        {
                            opening_percentage = 0.90;
                        }
                        if (dam_storage + dam_input > (weir_stor * 0.95))
                        {
                            opening_percentage = 1;
                        }

                        dam_outflow = rating_flow * opening_percentage;
                        ans[STATE_OPENING_PERCENTAGE] = opening_percentage - opening_percentage_initial;
                        ans[STATE_PIPE_FLOW] = dam_outflow - h6; // m3/s
                    ...
                    if (rating_flow > 1)
                    {
                        // if(debug1) printf("link : %f\n", i);
                        // if(debug1) printf("max : %f\n", q_max);
                        // if(debug1) printf("rating : %f\n", rating_flow);
